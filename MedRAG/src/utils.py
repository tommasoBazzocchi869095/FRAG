...
import tqdm
import numpy as np
from factuality import FactualityScorer
corpus_names = {
    "PubMed": ["pubmed"],
    "Textbooks": ["textbooks"],
...
class RetrievalSystem:
    ...
    def __init__(self, retriever_name="MedCPT", corpus_name="Textbooks", db_dir="./corpus", HNSW=False, cache=False, use_factuality=True):
        self.retriever_name = retriever_name
        self.corpus_name = corpus_name
        self.use_factuality = use_factuality
        ...
        if self.use_factuality:
            self.factuality_scorer = FactualityScorer(model_path="../Model/estimationFactualityOne/modello_finale_bertSourceBased")
        self.cache = cache
        if self.cache:
    ...
    def retrieve(self, question, k=32, rrf_k=100, id_only=False, alpha=0.5):
        '''
        ...
        if self.use_factuality and not output_id_only and len(merged_texts) > 0:
            print("enter in factuality block")
            docs_for_scoring = merged_texts
            if self.cache and self.docExt is not None:
                docs_for_scoring = self.docExt.extract(merged_texts)
            content_list = [item.get("contents", item.get("content", "")) for item in docs_for_scoring]
            fact_scores = self.factuality_scorer.predict(content_list)
            min_s = min(merged_scores)
            max_s = max(merged_scores)
            div = (max_s - min_s) if (max_s - min_s) > 1e-9 else 1.0
            norm_topic_scores = [(s - min_s) / div for s in merged_scores]
            final_scores = []
            for idx in range(len(merged_texts)):
                new_score = alpha * norm_topic_scores[idx] + (1 - alpha) * fact_scores[idx]
                if self.cache:
                    merged_texts[idx] = docs_for_scoring[idx]
                merged_texts[idx]["score_topic_orig"] = merged_scores[idx]
                merged_texts[idx]["score_factuality"] = float(fact_scores[idx])
                merged_texts[idx]["score_final"] = new_score
                final_scores.append(new_score)

            combined = list(zip(merged_texts, final_scores))
            combined.sort(key=lambda x: x[1], reverse=True)
            merged_texts = [x[0] for x in combined]
            merged_scores = [x[1] for x in combined]

        print("out")
        if self.use_factuality==False or len(merged_texts) <= 0:
