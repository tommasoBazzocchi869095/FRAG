...
retriever_name = "BM25"
CACHE_DIR = "."
results_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../MIRAGE/predictionRAGbm25"))
dataset_names = [ "mmlu", "medqa"]
...
for dataset_name in dataset_names:
    ...
    for idx, item in enumerate(tqdm(dataset_data)):
        ...
        if snippets:
            ...
            final_snippets.sort(key=lambda x: x["score_final"], reverse=True)
            top_k_snippets = final_snippets[:K]
            contexts = [f"Document [{i}] (Title: {s.get('title','')}) {s.get('content','')}" for i, s in enumerate(top_k_snippets)]
            context_str = "\n".join(contexts)
        else:
            context_str = ""
